    ...
    interface PersonActions {
        ...
        Function<Action<Person, Event>, Boolean> callAdult = action -> {
            Person adult = Simulation.getInstance().getCreatures().stream()
                            ...
                            .filter(person -> person.isAlive() && person.notBusy() && person.isAtHome() && person.getStatus() != FamilyStatus.KID)
                            .findFirst()
                            .orElse(null);

            if (adult != null) {
                adult.setRoom(action.getSubject().getOrigin());
                HelpFunctions.makeRecord(action.getExecutor(), String.format("Call %s to solve %s in %s", adult.getName(), action.getSubject().getEventType(), action.getSubject().getOrigin()));
                return true;
            } else {
